...
def weights(distribution, shape, dtype=tf.float32, initrange=1e-5,
    ...
    with tf.variable_scope(name):
        if distribution == 'norm':
            wghts = tf.Variable(
                initrange * tf.random_normal(shape, 0, 1, dtype, seed))
        elif distribution == 'tnorm':
            wghts = tf.Variable(
                initrange * tf.truncated_normal(shape, 0, 1, dtype, seed))
        elif distribution == 'uniform':
            wghts = tf.Variable(tf.random_uniform(
                shape, -initrange, initrange, dtype, seed))
        elif distribution == 'constant':
            wghts = tf.Variable(tf.constant(
                initrange, dtype=dtype, shape=shape))
        else:
            raise ValueError("Argument 'distribution takes values 'norm', 'tnorm', 'uniform', 'constant', "
                             "Received %s" % distribution)
        if l2 != 0.0:
            tf.add_to_collection('losses', tf.multiply(
                tf.nn.l2_loss(wghts), l2, name=name + 'weight_loss'))
        return wghts
...
def batch_normalize(tensor_in, epsilon=1e-5, decay=0.999):
    ...
    # [1] or [0], Using a placeholder to decide which
    is_training = tf.placeholder(tf.int32, shape=[None])
    ...
    pop_mean = tf.Variable(
        tf.zeros([tensor_in.get_shape()[-1]]), trainable=False)
    pop_var = tf.Variable(
        tf.ones([tensor_in.get_shape()[-1]]), trainable=False)
    ...
    # Choose statistic
    mean = tf.nn.embedding_lookup(
        tf.stack([running_mean, batch_mean]), is_training)
    variance = tf.nn.embedding_lookup(
        tf.stack([running_var, batch_variance]), is_training)

    shape = tensor_in.get_shape().as_list()
    gamma = tf.Variable(tf.constant(0.0, dtype=tf.float32,
                                    shape=[shape[1]], name='gamma'))
    beta = tf.Variable(tf.constant(1.0, dtype=tf.float32,
                                   shape=[shape[1]], name='beta'))
...
def bidir_lm_rnn(x, t, token_embed, layers, seq_len=None, context_vector=None, cell=tf.nn.rnn_cell.BasicLSTMCell):
    ...
    if context_vector is not None:
        input_features = [tf.concat([embedding, context_vector], 1)
    ...
    # 2  sen_len X num_users X hidden_size tensors
    f_hidden_states, b_hidden_states = tf.split(tf.stack(hidden_states), 2, 2)
    # truncate forward and backward output to align for prediction
    # sen_len-2 X num_users X hidden_size tensor
    f_hidden_states = tf.stack(tf.unstack(f_hidden_states)[:-2])
    # sen_len-2 X num_users X hidden_size tensor
    b_hidden_states = tf.stack(tf.unstack(b_hidden_states)[2:])
    # concatenate forward and backward output for prediction
    # sen_len-2 long list of num_users X 2*hidden_size tensors
    prediction_states = tf.unstack(
        tf.concat((f_hidden_states, b_hidden_states), 2))
    token_losses = batch_softmax_dist_loss(
        t, prediction_states, token_set_size)
...
def lm_rnn(x, t, token_embed, layers, seq_len=None, context_vector=None, cell=tf.nn.rnn_cell.BasicLSTMCell):
    ...
    # input_features: list max_length of sentence long tensors (mb X
    ...
    # cell_state: data structure that contains the cell state for each hidden
    # layer for a mini-batch (complicated)
...
def join_multivariate_inputs(feature_spec, specs, embedding_ratio,
                             max_embedding, min_embedding):
    """
    ...
    # Make placeholders for all input data and select embeddings for
    # categorical data
    for dataname in feature_spec['categorical']:
        embedding_size = math.ceil(
            embedding_ratio * specs[dataname]['num_classes'])
        embedding_size = int(
            max(min(max_embedding, embedding_size), min_embedding))
        with tf.variable_scope(dataname):
            placeholderdict[dataname] = tf.placeholder(tf.int32, [None])
            embedding_matrix = tf.Variable(
                1e-5 * tf.truncated_normal((specs[dataname]['num_classes'], embedding_size), dtype=tf.float32))
            embeddings[dataname] = tf.nn.embedding_lookup(
                embedding_matrix, placeholderdict[dataname])

    for dataname in feature_spec['continuous']:
        placeholderdict[dataname] = tf.placeholder(
            tf.float32, [None, len(specs[dataname]['index'])])
...
def softmax_dist_loss(truth, h, dimension, scale_range=1.0, U=None):
    ...
    y = tf.matmul(h, U) + b
    loss_column = tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=y, labels=truth)
...
def batch_softmax_dist_loss(truth, h, dimension, scale_range=1.0):
    ...
    # sequence_length X batch_size X final_hidden_size
    ...
    # sequence_length X final_hidden_size X dimension
    ustack = tf.stack([U] * len(h))
    # sequence_length X batch_size X dimension
    logits = tf.matmul(hidden_tensor, ustack) + b
    # batch_size X sequence_length X dimension
    logits = tf.transpose(logits, perm=[1, 0, 2])
    # added to store probabilities of true logline
    tf.add_to_collection("true_probabilities", tf.nn.softmax(logits))
    loss_matrix = tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=logits, labels=truth)  # batch_size X sequence_length
    return loss_matrix
...
def diag_mvn_loss(truth, h, scale_range=1.0, variance_floor=0.1):
    ...
    y = tf.matmul(h, U) + b
    # split y into two even sized matrices, each with half the columns
    mu, var = tf.split(y, 2, 1)
    # make the variance non-negative
    var = tf.maximum(tf.exp(var), variance_floor)
...
def full_mvn_loss(truth, h):
    ...
    z = tf.squeeze(tf.matrix_triangular_solve(var, tf.reshape(
        truth - mu, [-1, dimension, 1]), lower=True, adjoint=False))  # z should be MB x D
...
def multivariate_loss(h, loss_spec, placeholder_dict, variance_floor=0.01):
    ...
    for i, (input_name, loss_func, dimension) in enumerate(loss_spec):
        with tf.variable_scope(input_name):
            # this input will be a (classification or regression) target - need
            # to define a placeholder for it
            ...
            if loss_func == softmax_dist_loss:  # discrete
                component_wise_point_loss = loss_func(
                    x, h, dimension)  # MB X 1
            elif loss_func == diag_mvn_loss:  # continuous
                component_wise_point_loss, logdet = loss_func(
                    x, h, variance_floor=variance_floor)  # MB X DIM_MULTIVARIATE, MB X 1
                if logdet is not None:
                    log_det_list.append(logdet)
            else:  # continuous
                component_wise_point_loss, logdet = loss_func(
                    x, h)  # MB X DIM_MULTIVARIATE, MB X 1
                if logdet is not None:
                    log_det_list.append(logdet)
...
def layer_norm_rnn(inputs,
    ...
    layer_norm_stack = [nn.rnn_cell.BasicLSTMCell(
        layers[0], state_is_tuple=True)]
    for i in range(1, len(layers)):
        layer_norm_stack.append(
            tf.contrib.rnn.LayerNormBasicLSTMCell(layers[i]))
...
def swapping_rnn(inputs,
    ...
    cells = [nn.rnn_cell.BasicLSTMCell(
        num_units, state_is_tuple=True) for num_units in layers]
...
def _state_size_with_prefix(state_size, prefix=None):
    ...
    if prefix is not None:
        if not isinstance(prefix, list):
            raise TypeError(
                "prefix of _state_size_with_prefix should be a list.")
